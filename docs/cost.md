...
| Hosted Agent Brain | Same hosted billing axes as Hosted API | Hosted credit or your provider on matching BYO-key generation | Hosted retained `GB-month` | Lowest total product+ops cost when you want UI, source sync, and Brain management |
...
- If you want a hosted control plane for sources, memory, Playground, and code-aware Brain workflows, Agent Brain is usually cheaper than building those surfaces yourself.
...
| --- | --- | --- |
| AI generation | `ask`, `boolean_ask`, `code`, hosted Brain chat/code generation | Per 1,000 billable generation tokens |
...
### 4. Agent Brain reduces control-plane cost

Hosted Agent Brain does not only save runtime cost. It also saves build cost by giving you:
...
## Worked Example 3: Agent Brain Memory vs Full Transcript Replay
...
This is exactly why the `Remember` toggle in Agent Brain chat can be cheaper than dumping the entire transcript into every prompt.
...
## Worked Example 5: OSS vs Hosted vs Agent Brain
...
| Hosted Agent Brain | Same hosted usage axes as Hosted API | Hosted credit or your provider | Hosted retained `GB-month` | Very low | Lowest | Teams that want hosted source sync, Brain config, Playground, and code-aware workflows |
...
- Agent Brain is usually the cheapest path when you also value the hosted Brain workflow and do not want to build that surface area yourself
...
| Replay full chat history | Prompt size grows every turn | Use memory-backed chat / Agent Brain `Remember` | Context stays bounded instead of growing linearly forever |
...
| Build your own source sync | More engineering and more failure modes | Use hosted Brain sources or CLI ingest | Lowers ops and maintenance cost |
| Build your own memory UI and admin surface | Hidden product cost, not just model cost | Use Hosted Agent Brain / Dashboard | Cuts engineering time and support burden |
...
### Pick Hosted Agent Brain when

- you want hosted source sync, Brain config, Playground/Test Agent, and managed knowledge workflows
- you want to test chat, boolean ask, and code against the same Brain
...
| Hosted Agent Brain | You get the hosted runtime plus the Brain UI, source sync, Playground, and memory/code workflow without building those layers yourself |
