...
- existing shared SupaVector deployment
...
- if they already have `SUPAVECTOR_BASE_URL` and `SUPAVECTOR_API_KEY`, they are in a shared-deployment mode
- `--external-postgres` is still self-hosted SupaVector
- do not tell shared-deployment users to run local bootstrap or onboard commands on a client machine unless that machine is also the SupaVector server

## Local CLI First

If this repository is available locally and the goal is to run or test SupaVector on the same computer:

1. Prefer the `supavector` CLI over raw bootstrap commands.
2. Run `supavector doctor` first to see whether local prerequisites and saved config already exist.
3. If SupaVector has not been configured yet, run `supavector onboard`.
...
- On the normal local self-hosted path, `supavector onboard` creates the first admin and the first service token automatically.
- The CLI saves the base URL and service token in `~/.supavector/config.json`.
...
- Only surface the token with `supavector config show --show-secrets` when the user needs to wire another local app, backend, worker, or agent runtime.
- Never commit service tokens, JWTs, or provider keys to the repository.

## Collection Rules

- Use `--collection` explicitly on `supavector write`, `supavector search`, `supavector ask`, and `supavector boolean_ask` whenever collection scope matters.
- Keep write and read operations on the same collection unless the user explicitly wants a wider or different scope.
- For folder ingest, `supavector write --folder ./some-folder` uses the folder name as the collection if `--collection` is omitted.
- When a caller needs a grounded binary decision instead of a freeform answer, use `supavector boolean_ask`. The API, SDK, and UI responses include `supportingChunks` when the caller needs the exact evidence text.

## Model Rules

- For local self-hosted defaults, use `supavector changemodel` instead of telling the user to edit the env file by hand.
- `supavector onboard` and `supavector changemodel` support numbered provider choices first, then numbered model choices for the selected provider. Generation providers are currently OpenAI, Gemini, and Anthropic. Embedding providers are currently OpenAI and Gemini.
...
- `embedProvider` and `embedModel` are instance-wide, not tenant-specific. Changing either requires a reindex because SupaVector stores all vectors in one embedding space.
...
If the user is not self-hosting locally and is targeting an existing SupaVector deployment instead:

- use `SUPAVECTOR_BASE_URL` and `SUPAVECTOR_API_KEY`
